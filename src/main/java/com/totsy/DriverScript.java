...
import com.totsy.read.Xls_Reader;
import com.totsy.test.Constants;
import com.totsy.test.Keywords;
...
public class DriverScript {
	...
	public static void main(String[] args) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, IOException, NoSuchMethodException, SecurityException {
		FileInputStream fs = new FileInputStream("src/main/java/com/totsy/config/config.properties");
		CONFIG= new Properties();
		CONFIG.load(fs);

		fs = new FileInputStream("src/main/java/com/totsy/config/or.properties");
		OR= new Properties();
		OR.load(fs);
	...
	public void startTest() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException{
		// initialize the app logs
		APP_LOGS = Logger.getLogger("akwadhwaLogger");
		...
		suiteXLS = new Xls_Reader("src/main/java/com/totsy/xls/Suite.xlsx");
		...
		for(currentSuiteID=2;currentSuiteID<=suiteXLS.getRowCount(Constants.TEST_SUITE_SHEET);currentSuiteID++){
			...
			if(suiteXLS.getCellData(Constants.TEST_SUITE_SHEET, Constants.RUNMODE, currentSuiteID).equals(Constants.RUNMODE_YES)){
				...
				currentTestSuiteXLS=new Xls_Reader("src/main/java/com/totsy/xls/"+currentTestSuite+".xlsx");
